define(['jquery', 'kind', 'cui', 'guid'], function ($, kind, cui, guid) {
    ...
    priv.setup = function _setup (dd) {
        ...
        // Create a jQuery version of the element
        dd.elems.$toggleWrap = $(dd.elems.toggleWrap);
    ...
    priv.show = function _show (dd) {
        ...
        for (id in dropdowns) {
            if (id !== dd.sourceElem.id && dropdowns.hasOwnProperty(id) && dropdowns[id]) {
                if (!dropdowns[id].isCollapsed) {
                    priv.hide(dropdowns[id]);
                }
            }
        }
        ...
        $body.on('click.dropdown', function _Dropdown_body_onClick (evt) {
    ...
    priv.positionPopover = function _positionPopover (dd) {
        ...
        var togglerOffset = dd.elems.$toggleWrap.offset();

        // Small screens
        if (priv.isSmallScreen(dd)) {
            //FIXME: Center it on the viewport -- or use the Modal component
