...
The UI Position plugin allows for any element to be positioned on the page with a variety of options.  


## Positioning
The UI Position plugin allows you to position elements absolutely or with respect to another valid element. 
...
`overlay` | Mixed | An object or boolean that defines the overlay shield. Default: `true` (displays a transparent overlay). See [overlay options](#overlay-options).
...
Event type | Description
--- | --- | ---
`show.cui.modal` | This fires immediately when the popover begins to appear (e.g. when clicking the button or calling `mymodal.show()`)
`shown.cui.modal` | This fires after the popover is fully displayed (i.e. after CSS animations have completed)
`hide.cui.modal` | This fires immediately when the popover begins to hide (e.g. when clicking outside the modal or calling `mymodal.hide()`)
`hidden.cui.modal` | This fires after the popover is fully hidden (i.e. after CSS animations have completed)
